...
static int spdif_suspend(struct snd_soc_dai *cpu_dai)
{
	...
	spdif->saved_clkcon = readl(regs	+ CLKCON);
	spdif->saved_con = readl(regs + CON);
	spdif->saved_cstas = readl(regs + CSTAS);
...
static __devinit int spdif_probe(struct platform_device *pdev)
{
	...
	/* Get SPDIF power domain regulator */
	spdif->regulator = regulator_get(&pdev->dev, "pd");
	if (IS_ERR(spdif->regulator)) {
		dev_err(&pdev->dev, "%s: failed to get resource %s\n",
				__func__, "spdif");
		return PTR_ERR(spdif->regulator);
	}

	/* Enable Power domain */
	regulator_enable(spdif->regulator);
	...
	clk_set_parent(spdif->sclk, mout_audio);
