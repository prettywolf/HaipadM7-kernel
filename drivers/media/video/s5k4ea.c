...
enum {
	/* S5K4EA_PREVIEW_WVGA, */
	S5K4EA_PREVIEW_HD1080,
};

struct s5k4ea_enum_framesize {
	unsigned int index;
	unsigned int width;
	unsigned int height;
};

struct s5k4ea_enum_framesize s5k4ea_framesize_list[] = {
	/* { S5K4EA_PREVIEW_WVGA,  800,  480 }, */
	{ S5K4EA_PREVIEW_HD1080, 1920, 1080 }
};
					...
					struct v4l2_frmsizeenum *fsize)
{
	...
	int num_entries = sizeof(s5k4ea_framesize_list) /
				sizeof(struct s5k4ea_enum_framesize);
	struct s5k4ea_enum_framesize *elem;
	int index = 0;
	int i = 0;
	...
	for (i = 0; i < num_entries; i++) {
		elem = &s5k4ea_framesize_list[i];
		if (elem->index == index) {
			fsize->discrete.width =
			    s5k4ea_framesize_list[index].width;
			fsize->discrete.height =
			    s5k4ea_framesize_list[index].height;
			return 0;
		}
	}
