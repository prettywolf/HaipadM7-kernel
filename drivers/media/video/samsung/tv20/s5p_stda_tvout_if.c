...
#define TVOUTIFPRINTK(fmt, args...) \
	pr_debug(KERN_INFO "\t[TVOUT_IF] %s: " fmt, __func__ , ## args)
#else
#define TVOUTIFPRINTK(fmt, args...)
#endif
...
static bool s5p_tv_if_init_vm_reg(void)
{
	...
	merr = s5p_vm_init_status_reg(burst, endian);
	if (merr != VMIXER_NO_ERROR) {
		pr_err("%s::s5p_vm_init_status_reg(%d, %d) fail\n",
			__func__, burst, endian);
		return false;
	}

	merr = s5p_vm_init_display_mode(disp_mode, out_mode);
	if (merr != VMIXER_NO_ERROR) {
		pr_err("%s::s5p_vm_init_display_mode(%d, %d) fail\n",
			__func__, disp_mode, out_mode);
		return false;
	}

	s5p_vm_init_bg_dither_enable(cr_en, cb_en, y_en);

	for (i = VMIXER_BG_COLOR_0; i <= VMIXER_BG_COLOR_2; i++) {
		merr = s5p_vm_init_bg_color(i,
				     st->bg_color[i].color_y,
				     st->bg_color[i].color_cb,
				     st->bg_color[i].color_cr);

		if (merr != VMIXER_NO_ERROR) {
			pr_err("%s::s5p_vm_init_bg_color(%d, %d %d) fail\n",
			__func__,
			st->bg_color[i].color_y,
			st->bg_color[i].color_cb,
			st->bg_color[i].color_cr);
	...
	case TVOUT_OUTPUT_COMPONENT_RGB_PROGRESSIVE:
		s5p_vm_init_csc_coef_default(VMIXER_CSC_RGB_TO_YUV601_FR);
		break;
	...
	case TVOUT_OUTPUT_DVI:
		...
		break;
	default:
		pr_err("%s::invalid tvout_param.out_mode parameter(%d)\n",
			__func__, out_mode);
		return false;
		break;
	}
...
static bool s5p_tv_if_init_sd_reg(void)
{
	...
	if (sderr != SDOUT_NO_ERROR) {
		pr_err("%s::s5p_sdout_init_display_mode() fail\n", __func__);
		return false;
	}
	...
	sderr = s5p_sdout_init_video_scale_cfg(cpn_lev, cpn_rat,
						cps_lev, cps_rat);
	if (sderr != SDOUT_NO_ERROR) {
		pr_err("%s::s5p_sdout_init_video_scale_cfg() fail\n", __func__);
		return false;
	}

	sderr = s5p_sdout_init_sync_signal_pin(sync_pin);
	if (sderr != SDOUT_NO_ERROR) {
		pr_err("%s::s5p_sdout_init_sync_signal_pin() fail\n", __func__);
		return false;
	}
	...
	sderr = s5p_sdout_init_vbi(wss_cvbs, cap_cvbs, wss_y_svideo,
		...
		wss_y, cap_y_pb_pr);
	if (sderr != SDOUT_NO_ERROR) {
		pr_err("%s::s5p_sdout_init_vbi() fail\n", __func__);
		return false;
	}
	...
	for (i = SDOUT_CHANNEL_0; i <= SDOUT_CHANNEL_2; i++) {
		...
		if (sderr != SDOUT_NO_ERROR) {
			pr_err("%s::s5p_sdout_init_offset_gain() fail\n", __func__);
			return false;
		}
	}
	...
	s5p_sdout_init_component_porch(back_525, front_525, back_625, front_625);

	sderr = s5p_sdout_init_vesa_rgb_sync(sync_type, vsync_active, hsync_active);
	if (sderr != SDOUT_NO_ERROR) {
		pr_err("%s::s5p_sdout_init_vesa_rgb_sync() fail\n", __func__);
		return false;
	}
	...
	for (i = SDOUT_CHANNEL_0; i <= SDOUT_CHANNEL_2; i++) {
		...
		sderr = s5p_sdout_init_ch_xtalk_cancel_coef(channel, coeff2, coeff1);
		if (sderr != SDOUT_NO_ERROR) {
			pr_err("%s::s5p_sdout_init_ch_xtalk_cancel_coef() fail\n",
				__func__);
			return false;
		}
	}
	...
	if (sderr != SDOUT_NO_ERROR) {
		pr_err("%s::s5p_sdout_init_wss525_data() fail\n", __func__);
		return false;
	}
	...
	sderr = s5p_sdout_init_wss625_data(w6_surroun_f_sound, w6_copyright,
					...
					w6_display_ratio);
	if (sderr != SDOUT_NO_ERROR) {
		pr_err("%s::s5p_sdout_init_wss625_data() fail\n", __func__);
		return false;
	}
	...
	sderr = s5p_sdout_init_cgmsa525_data(c5_copy_permit, c5_mv_psp,
					...
					c5_display_ratio);
	if (sderr != SDOUT_NO_ERROR) {
		pr_err("%s::s5p_sdout_init_cgmsa525_data() fail\n", __func__);
		return false;
	}
	...
	sderr = s5p_sdout_init_cgmsa625_data(c6_surroun_f_sound, c6_copyright,
					...
					c6_display_ratio);
	if (sderr != SDOUT_NO_ERROR) {
		pr_err("%s::s5p_sdout_init_cgmsa625_data() fail\n", __func__);
		return false;
	}
	...
	for (i = SDOUT_CHANNEL_0; i <= SDOUT_CHANNEL_2; i++) {
		...
		if (sderr != SDOUT_NO_ERROR) {
			pr_err("%s::s5p_sdout_init_dac_power_onoff() fail\n",
				__func__);
			return false;
		}
	}
...
static bool s5p_tv_if_init_hd_video_reg(void)
{
	...
	if (!s5p_tv_if_init_avi_frame(&st->tvout_param)) {
		pr_err("%s::s5p_tv_if_init_avi_frame() fail\n",
			__func__);
	...
	if (herr != HDMI_NO_ERROR) {
		pr_err("%s::s5p_hdmi_video_init_display_mode() fail\n",
			__func__);
		return false;
	}
	...
	if (herr != HDMI_NO_ERROR) {
		pr_err("%s::s5p_hdmi_video_init_csc() fail\n", __func__);
		return false;
	}
	...
	herr =  s5p_hdmi_video_init_avi_infoframe(*a_trans_type,
					*a_check_sum, a_data);
	if (herr != HDMI_NO_ERROR) {
		pr_err("%s::s5p_hdmi_video_init_avi_infoframe() fail\n", __func__);
		return false;
	}
	...
	herr = s5p_hdmi_video_init_mpg_infoframe(m_trans_type,
					m_check_sum, m_data);
	if (herr != HDMI_NO_ERROR) {
		pr_err("%s::s5p_hdmi_video_init_mpg_infoframe() fail\n", __func__);
		return false;
	}
	...
	herr = s5p_hdmi_init_spd_infoframe(s_trans_type,
					spd_header, spd_data);
	if (herr != HDMI_NO_ERROR) {
		pr_err("%s::s5p_hdmi_init_spd_infoframe() fail\n", __func__);
		return false;
	}
