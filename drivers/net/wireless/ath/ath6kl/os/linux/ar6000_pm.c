...
static void ar6000_wow_resume(struct ar6_softc *ar)
{
    if (ar->arWowState!= WLAN_WOW_STATE_NONE) {
        ...
        struct wmi_set_host_sleep_mode_cmd hostSleepMode = {true, false};
...
#define WOW_LIST_ID 1
    if (ar->arNetworkType != AP_NETWORK) {
        ...
        */
        ...
        int status;
        struct wmi_add_wow_pattern_cmd addWowCmd = { .filter = { 0 } };
        struct wmi_del_wow_pattern_cmd delWowCmd;
        struct wmi_set_host_sleep_mode_cmd hostSleepMode = {false, true};
        struct wmi_set_wow_mode_cmd wowMode = {    .enable_wow = true,
        ...
        status = hif_wait_for_pending_recv(ar->arHifDevice);
...
int
ar6000_setup_cut_power_state(struct ar6_softc *ar,  enum ar6k_wlan_state state)
{
    int                      status = 0;
    ...
    do {
        if (state == WLAN_ENABLED) {
            ...
            status = hif_config_dev(ar->arHifDevice,
...
int
ar6000_setup_deep_sleep_state(struct ar6_softc *ar, enum ar6k_wlan_state state)
{
    int status = 0;
...
#endif
    do {
        struct wmi_set_host_sleep_mode_cmd hostSleepMode;
        ...
        if (state == WLAN_ENABLED) {
            ...
            struct wmi_set_wow_mode_cmd wowMode = { .enable_wow = false };
...
int
ar6000_update_wlan_pwr_state(struct ar6_softc *ar, enum ar6k_wlan_state state, bool pmEvent)
{
    int status = 0;
    u16 powerState, oldPowerState;
    enum ar6k_wlan_state oldstate = ar->arWlanState;
    ...
    if (status) {
        ...
        struct wmi_report_sleep_state_event  wmiSleepEvent, *pSleepEvent = NULL;
...
int
ar6000_set_wlan_state(struct ar6_softc *ar, enum ar6k_wlan_state state)
{
    int status;
