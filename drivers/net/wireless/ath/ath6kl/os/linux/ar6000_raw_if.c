...
ar6000_htc_raw_read_cb(void *Context, struct htc_packet *pPacket)
{
    ...
    struct raw_htc_buffer    *busy;
    enum htc_raw_stream_id streamID; 
    struct ar6_raw_htc *arRaw = ar->arRawHtc;

    busy = (struct raw_htc_buffer *)pPacket->pPktContext;
    A_ASSERT(busy != NULL);
...
{
    struct ar6_softc          *ar = (struct ar6_softc  *)Context;
    struct raw_htc_buffer      *free;
    enum htc_raw_stream_id   streamID;
    struct ar6_raw_htc *arRaw = ar->arRawHtc;
    
    free = (struct raw_htc_buffer *)pPacket->pPktContext;
    A_ASSERT(free != NULL);
...
int ar6000_htc_raw_open(struct ar6_softc *ar)
{
    ...
    struct raw_htc_buffer *buffer;
    u16 servicepriority;
    struct ar6_raw_htc *arRaw = ar->arRawHtc;
    if (!arRaw) {
        arRaw = ar->arRawHtc = kmalloc(sizeof(struct ar6_raw_htc), GFP_KERNEL);
        if (arRaw) {
            memset(arRaw, 0, sizeof(struct ar6_raw_htc));
        }
    }
	...
	sema_init(&arRaw->raw_htc_write_sem[streamID], 1);
        ...
        for (count2 = 0; count2 < RAW_HTC_READ_BUFFERS_NUM; count2 ++) {
            ...
            memset(buffer, 0, sizeof(struct raw_htc_buffer));
...
get_filled_buffer(struct ar6_softc *ar, enum htc_raw_stream_id StreamID)
{
    int count;
    struct raw_htc_buffer *busy;
    struct ar6_raw_htc *arRaw = ar->arRawHtc;
...
ssize_t ar6000_htc_raw_read(struct ar6_softc *ar, enum htc_raw_stream_id StreamID, 
...
static struct raw_htc_buffer *
get_free_buffer(struct ar6_softc *ar, enum htc_endpoint_id StreamID)
{
    int count;
    struct raw_htc_buffer *free;
    struct ar6_raw_htc *arRaw = ar->arRawHtc;
...
ssize_t ar6000_htc_raw_write(struct ar6_softc *ar, enum htc_raw_stream_id StreamID,
                     char __user *buffer, size_t length)
{
    int writePtr;
    struct raw_htc_buffer *free;
