                      ...
                      struct cfg80211_connect_params *sme)
{
    ...
    if(sme->bssid){
        if(memcmp(&sme->bssid, bcast_mac, ETH_ALEN)) {
        ...
        ((WPA_PSK_AUTH == ar->arAuthMode) || (WPA2_PSK_AUTH == ar->arAuthMode)))
    {
		...
		(jiffies + HZ * (A_DISCONNECT_TIMER_INTERVAL) / 1000));
    }
                ...
                u8 assocRespLen, u8 *assocInfo)
{
    ...
    if (ADHOC_NETWORK & networkType) {
		    ...
		    *ptr_ie_buf++ = 0; /* ATIM window */
            ...
            memcpy(source_mac, ar->arNetDev->dev_addr, ETH_ALEN);
            ptr_ie_buf = ie_buf;
    } else {
                      ...
                      struct key_params *params)
{
    ...
    memset(key, 0, sizeof(struct ar_key));
...
{
    struct ar6_softc *ar = ar6k_priv(dev);
    struct wmi_power_mode_cmd pwrMode;
                        ...
                        struct cfg80211_ibss_params *ibss_param)
{
    ...
    memset(ar->arReqBssid, 0, sizeof(ar->arReqBssid));
    if(ibss_param->bssid) {
        if(memcmp(&ibss_param->bssid, bcast_mac, ETH_ALEN)) {
