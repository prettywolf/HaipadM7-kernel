...
static int HifReadWriteScatter(struct hif_device *device, struct hif_scatter_req *pReq)
{
    ...
    do {
        ...
        hif_add_to_async_list(device, pReqPriv->busrequest);
...
int SetupHIFScatterSupport(struct hif_device *device, struct hif_device_scatter_support_info *pInfo)
{
    ...
    do {
        ...
        for (i = 0; i < MAX_SCATTER_REQUESTS; i++) {    
            ...
            pReqPriv = (struct hif_scatter_req_priv *)kzalloc(sizeof(struct hif_scatter_req_priv), GFP_KERNEL);
            if (NULL == pReqPriv) {
                break;    
            }
            memset(pReqPriv, 0, sizeof(struct hif_scatter_req_priv));
                /* save the device instance*/
            pReqPriv->device = device;      
                /* allocate the scatter request */
            pReqPriv->pHifScatterReq = (struct hif_scatter_req *)kzalloc(sizeof(struct hif_scatter_req) + (MAX_SCATTER_ENTRIES_PER_REQ - 1) * (sizeof(struct hif_scatter_item)), GFP_KERNEL); 
            ...
            memset(pReqPriv->pHifScatterReq, 0, sizeof(struct hif_scatter_req));
    ...
    while (1) {
        ...
        if (pReqPriv->busrequest != NULL) {
            pReqPriv->busrequest->pScatterReq = NULL;
                /* free bus request */
            hif_free_bus_req(device, pReqPriv->busrequest);
