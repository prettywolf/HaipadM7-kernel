...
#endif /* CONFIG_PM */
static void hif_del_dev(struct hif_device * device);
static int hif_func0_cmd52_wr_byte(struct mmc_card *card, unsigned int address, unsigned char byte);
static int hif_func0_cmd52_rd_byte(struct mmc_card *card, unsigned int address, unsigned char *byte);
			       ...
			       const struct sdio_device_id *id)
{
	...
	for (count = 0; count < BUS_REQUEST_MAX_NUM; count++) {
		...
		hif_free_bus_req(device, &device->busRequest[count]);
	}
...
static void ath6kl_hifdev_remove(struct sdio_func *func)
{
	...
	if (device->claimedContext != NULL)
		status = ar6000_unavail_ev(device->claimedContext, device);
...
static int ath6kl_hifdev_suspend(struct device *dev)
{
	...
	if (device && device->claimedContext) {
		...
		status = ar6000_suspend_ev(device->claimedContext);
		if (status)
...
static int ath6kl_hifdev_resume(struct device *dev)
{
	...
	device = ath6kl_get_hifdev(func);
	if (device && device->claimedContext) {
		status = ar6000_resume_ev(device->claimedContext);
		if (status == 0)
			device->is_suspend = false;
	}
...
static void SetExtendedMboxWindowInfo(u16 Manfid, struct hif_device_mbox_info *pInfo)
{
...
static int
__hif_read_write(struct hif_device *device,
             u32 address,
             u8 *buffer,
             u32 length,
             u32 request,
    ...
    do {
        ...
        if (request & HIF_WRITE) {
            ...
            if (BUFFER_NEEDS_BOUNCE(buffer)) {
                BUG_ON(device->dma_buffer == NULL);
                tbuffer = device->dma_buffer;
                    /* copy the write data to the dma buffer */
                BUG_ON(HIF_DMA_BUFFER_SIZE <= length);
             ...
             void *context)
{
    ...
    do {            
        if ((request & HIF_ASYNCHRONOUS) || (request & HIF_SYNCHRONOUS)){
            ...
            busrequest = hif_alloc_bus_req(device);
            if (busrequest == NULL) {
                AR_DEBUG_PRINTF(ATH_DEBUG_ERROR, 
            ...
            hif_add_to_async_list(device, busrequest);
            ...
            if (request & HIF_SYNCHRONOUS) {
                ...
                if (down_interruptible(&busrequest->sem_req) != 0) {
                    ...
                    hif_free_bus_req(device, busrequest);
                    return status;
                }
            } else {
        ...
        while (device->asyncreq != NULL) {
            ...
            if (request->pScatterReq != NULL) {
                ...
                status = DoHifReadWriteScatter(device,request);
                    ...
                    /* call hif_read_write in sync mode to do the work */
                ...
                if (request->request & HIF_ASYNCHRONOUS) {
                    ...
                    hif_free_bus_req(device, request);
...
int ReinitSDIO(struct hif_device *device)
{
    ...
    do {
        ...
        if (card->host->caps & MMC_CAP_SD_HIGHSPEED) {
            ...
            err = hif_func0_cmd52_rd_byte(card, SDIO_CCCR_SPEED, &cmd52_resp);
            if (err) {
                AR_DEBUG_PRINTF(ATH_DEBUG_ERR, ("ReinitSDIO: CMD52 read to CCCR speed register failed  : %d \n",err));    
                card->state &= ~MMC_STATE_HIGHSPEED;
                /* no need to break */
            } else {
                err = hif_func0_cmd52_wr_byte(card, SDIO_CCCR_SPEED, (cmd52_resp | SDIO_SPEED_EHS));
                if (err) {
        ...
        if (card->host->caps & MMC_CAP_4_BIT_DATA) {
            ...
            err = hif_func0_cmd52_wr_byte(card, SDIO_CCCR_IF, SDIO_BUS_CD_DISABLE | SDIO_BUS_WIDTH_4BIT);
            if (err) {
...
void
hif_shutdown_dev(struct hif_device *device)
{
    AR_DEBUG_PRINTF(ATH_DEBUG_TRACE, ("AR6000: +hif_shutdown_dev\n"));
    if (device != NULL) {
        BUG_ON(device->func == NULL);
    } else {
             ...
             * enumerated */
        AR_DEBUG_PRINTF(ATH_DEBUG_TRACE, ("AR6000: hif_shutdown_dev, resetting\n"));
        ResetAllCards();
...
static int enable_task(void *param)
{
	...
	ar6000_power_change_ev(device->claimedContext, HIF_DEVICE_POWER_UP) != 0)
	    AR_DEBUG_PRINTF(ATH_DEBUG_TRACE, ("AR6000: Device rejected\n"));
...
void
hif_ack_interrupt(struct hif_device *device)
{
    BUG_ON(device == NULL);
...
void
hif_unmask_interrupt(struct hif_device *device)
{
    int ret;
    ...
    AR_DEBUG_PRINTF(ATH_DEBUG_TRACE, ("AR6000: hif_unmask_interrupt\n"));
...
void hif_mask_interrupt(struct hif_device *device)
{
    ...
    AR_DEBUG_PRINTF(ATH_DEBUG_TRACE, ("AR6000: hif_mask_interrupt\n"));
...
struct bus_request *hif_alloc_bus_req(struct hif_device *device)
{
    ...
    AR_DEBUG_PRINTF(ATH_DEBUG_TRACE, ("AR6000: hif_alloc_bus_req: 0x%p\n", busrequest));
    return busrequest;
}

void
hif_free_bus_req(struct hif_device *device, struct bus_request *busrequest)
{
    unsigned long flag;

    BUG_ON(busrequest == NULL);
    AR_DEBUG_PRINTF(ATH_DEBUG_TRACE, ("AR6000: hif_free_bus_req: 0x%p\n", busrequest));
    /* Acquire lock */
    spin_lock_irqsave(&device->lock, flag);
...
static int hif_disable_func(struct hif_device *device, struct sdio_func *func)
{
    ...
    AR_DEBUG_PRINTF(ATH_DEBUG_TRACE, ("AR6000: +hif_disable_func\n"));
    device = ath6kl_get_hifdev(func);
    ...
    if (reset_sdio_on_unload) {
        ...
        ret = hif_func0_cmd52_wr_byte(device->func->card, SDIO_CCCR_ABORT, (1 << 3)); 
        if (ret) {
            status = A_ERROR;
...
static int hif_enable_func(struct hif_device *device, struct sdio_func *func)
{
    ...
    AR_DEBUG_PRINTF(ATH_DEBUG_TRACE, ("AR6000: +hif_enable_func\n"));
    device = ath6kl_get_hifdev(func);
    ...
    if (device->is_disabled) {
       /* enable the SDIO function */
        ...
        if ((device->id->device & MANUFACTURER_ID_AR6K_BASE_MASK) >= MANUFACTURER_ID_AR6003_BASE) {
            ...
            ret = hif_func0_cmd52_wr_byte(func->card, CCCR_SDIO_IRQ_MODE_REG, SDIO_IRQ_MODE_ASYNC_4BIT_IRQ);
            if (ret) {
...
static void
hif_del_dev(struct hif_device * device)
{
    BUG_ON(device == NULL);
    AR_DEBUG_PRINTF(ATH_DEBUG_TRACE, ("AR6000: hif_del_dev; 0x%p\n", device));
...
void hif_claim_dev(struct hif_device  *device, void *context)
{
...
{
    memset(&device->htcCallbacks, 0, sizeof(device->htcCallbacks));
}
